...
// Фирменный цвет и локализация ролей
const BRAND = "#8d2828";
...
// Конфигурация плиток
type NavItem = { href?: string; label: string; adminOnly?: boolean; action?: () => void };

function useNavConfig(onPurgeOpen: () => void): { common: NavItem[]; admin: NavItem[] } {
  const common: NavItem[] = [
    { href: "/dashboard", label: "Основное" },
    { href: "/chat", label: "Чаты" },
    { href: "/inboxTasks", label: "Задачи" },
    { href: "/calendar", label: "Календарь" },
    { href: "/schedule", label: "Расписание" },
    { href: "/teachers", label: "Педагоги" },
    { href: "/discussions", label: "Тренды" },
    { href: "/archive_tasks", label: "Архив задач" },
  ];

  const admin: NavItem[] = [
    { href: "/admin/groups", label: "Кафедры и группы", adminOnly: true },
    { label: "Очистка базы", adminOnly: true, action: onPurgeOpen },
  ];

  return { common, admin };
}

// Разбиение ФИО: Фамилия caps + остальное
...
function NavTile(props: {
  ...
  const body = (
    <div className={`tile glass ${active ? "active" : ""} ${hasUnread && !active ? "unread" : ""}`} role="button">
...
export default function Sidebar() {
  ...
  // Счётчик «задачи, назначенные мне»
  const [tasksToMe, setTasksToMe] = useState(0);
  const assigneeIdsOf = (t: TaskLite) => {
  ...
  const myAssigneeStatus = (t: TaskLite, myId?: string | null) => {
    if (!myId) return null;
    const rec = (t.assignees || []).find(a => a.userId === myId);
    return rec?.status ?? null;
    };
  const tasksFromLocal = (id: string) => {
    try {
  ...
  const tasksFromServer = useCallback(async (id: string) => {
    try {
  ...
  const openPurgeModal = () => setConfirmOpen(true);
  const nav = useNavConfig(openPurgeModal);

  // Подписки и первичные запросы
  useEffect(() => {
  ...
  }, [authed, uid, refreshUnread, tasksFromServer]);
    ...
    <aside className="wrap">
          ...
          <>
            <div className="navGrid">
              {nav.common.map((it) => {
                ...
                const isTasks = it.href === "/inboxTasks";
                const unread = isChat ? (pathname !== "/chat" ? unreadTotal : 0) : isTasks ? (pathname !== "/inboxTasks" ? tasksToMe : 0) : null;
                const active = it.href ? pathname === it.href : false;
                if (it.action) {
                  return <NavTile key={it.label} label={it.label} onClick={it.action} />;
                }
                return <NavTile key={it.label} label={it.label} href={it.href} active={active} unread={unread} />;
              })}
            </div>
              ...
              <>
                ...
                <div className="navGrid">
                  {nav.admin.map((it) => {
                    ...
                    return <NavTile key={it.label} label={it.label} href={it.href} active={active} />;
                  })}
                </div>
              </>
            )}
      ...
      {confirmOpen && (
        <div className="modal-backdrop" role="dialog" aria-modal="true" onClick={() => !cleaning && setConfirmOpen(false)}>
          <div className="modal-card" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="modal-text">Вы собираетесь очистить базу от архивированных и удаленных пользователей. Это действие необратимо.</p>
      ...
      <style jsx>{`
        .wrap {
          ...
          height: 100%;
          width: 256px; /* фикс ширины — как в рабочей сборке */
          background: #fff;
