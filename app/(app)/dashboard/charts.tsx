...
import type { DayPoint, TodaySlice, Priorities, WeekdayLoad } from './types';
import s from './page.module.css';

/** хук измерения контейнера */
function useMeasure() {
...
/** Линейный график: «Создано / Выполнено» */
...
/** Бар-чарт по today/overdue/upcoming */
...
/** Кольцевая диаграмма приоритетов */
export function PrioritiesDonut({ data }: { data: Priorities }) {
  const total = Math.max(1, data.high + data.normal);
  const aHigh = (data.high / total) * 2 * Math.PI;
  const aNorm = (data.normal / total) * 2 * Math.PI;
  ...
  return (
    <div className={s.chartBox} style={{ height: size }}>
      <svg width={width} height={size}>
        ...
        <path d={arcPath(-Math.PI / 2, aHigh)} fill="var(--brand)" opacity={0.9} />
        <path d={arcPath(-Math.PI / 2 + aHigh, aNorm)} fill="#111827" opacity={0.9} />
        <circle cx={cx} cy={cy} r={r - 26} fill="white" />
        <text x={cx} y={cy - 4} textAnchor="middle" fontSize="16" fill="#111827" fontWeight={700}>{total}</text>
        <text x={cx} y={cy + 14} textAnchor="middle" fontSize="12" fill="#6b7280">всего активных</text>

        <g transform={`translate(${190}, ${cy - 18})`}>
          <rect x={0} y={-10} width={10} height={10} fill="var(--brand)" />
          <text x={18} y={0} fontSize="12" fill="#111827">
            Срочно — {data.high} ({pct(data.high)}%)
          </text>
          <rect x={0} y={14} width={10} height={10} fill="#111827" />
          <text x={18} y={24} fontSize="12" fill="#111827">
            Обычная задача — {data.normal} ({pct(data.normal)}%)
          </text>
        </g>
      </svg>
    </div>
