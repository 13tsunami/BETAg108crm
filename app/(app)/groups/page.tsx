...
export default async function Page({
  ...
  const [users, groups, subjects] = await Promise.all([
    prisma.user.findMany({
      where: qu ? { name: { contains: qu, mode: 'insensitive' } } : undefined,
      select: { id: true, name: true, role: true },
      orderBy: [{ name: 'asc' }],
    }),
    prisma.group.findMany({
      where: qg ? { name: { contains: qg, mode: 'insensitive' } } : undefined,
      select: { id: true, name: true },
      orderBy: [{ name: 'asc' }],
    }),
    prisma.subject.findMany({
      where: qs ? { name: { contains: qs, mode: 'insensitive' } } : undefined,
