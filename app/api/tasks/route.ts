...
// Региональные подсказки Vercel (Neon у тебя в eu-central-1 → ближе fra1/cdg1/arn1):
...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error("GET /api/tasks failed:", {
      message: e?.message,
      code: e?.code,
      meta: e?.meta,
      stack: e?.stack,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const description =
      typeof (body as any).description === "string" ? (body as any).description : "";
    const priority =
      typeof (body as any).priority === "string" && (body as any).priority.trim()
    ...
    // Гарантируем наличие тегов (без интерактивной транзакции)
    ...
    if (tagNames.length > 0) {
      ...
      const existingMap = new Map<string, string>(
        existing.map((t: Pick<Tag, "id" | "name">) => [t.name.toLowerCase(), t.id]),
      );
    ...
    if (tagsToLink.length > 0) {
      await prisma.taskTag.createMany({
        data: tagsToLink.map((t) => ({ taskId: created.id, tagId: t.id })),
        skipDuplicates: true,
      });
    }
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    console.error("PATCH /api/tasks failed:", {
      message: e?.message,
      code: e?.code,
      meta: e?.meta,
      stack: e?.stack,
